...
def _prepare_db(path: str) -> None:
    ...
    conn.execute(
        "CREATE TABLE player_stats_weekly (player_id VARCHAR, season INTEGER, week INTEGER, fantasy_points DOUBLE, position VARCHAR)"
    )
...
class _FakeBuilder:
    ...
    def build_all_features(
        self, current_class_year: int, positions: list[str] | None = None
        ...
        return [
            ProspectFeatures(
                ...
                height=75,
                vertical=35,
                college_completion_pct_proxy=0.68,
                college_ypa=9.2,
                college_pass_td_rate=0.082,
                college_qb_rush_ypg=48.0,
            ...
            ProspectFeatures(
                ...
                height=73,
                vertical=29,
                college_completion_pct_proxy=0.56,
                college_ypa=6.7,
                college_pass_td_rate=0.039,
                college_qb_rush_ypg=11.0,
            ...
            ProspectFeatures(
                ...
                height=75,
                vertical=36,
                college_completion_pct_proxy=0.66,
                college_ypa=8.8,
                college_pass_td_rate=0.074,
                college_qb_rush_ypg=42.0,
                college_scramble_rate=0.09,
                adp=2.0,
            ),
        ]
        ...
        return (
            [
                ...
                HistoricalComp(
                    ...
                    match_reason="similar age profile",
                ),
                HistoricalComp(
                    player_id="hist_bust",
                    player_name="Bust",
                    role="floor",
                    outcome_bucket="bust",
                    match_reason="similar athleticism profile",
                ),
            ],
            False,
        )
...
class _FakeDivergence:
    def compute_sub_flags(self, prospect, comp_features):
        return [
            SubFlag(
                signal_name="Draft Capital",
                direction="positive",
                magnitude_str="closer to round one",
            )
        ]

    def compute_divergence(
        self, player_id, model_rankings, adp_rankings, low_confidence
    ):
...
def test_run_pipeline_persists_outputs(monkeypatch, tmp_path):
    ...
    output_count = conn.execute(
        "SELECT COUNT(*) FROM prospect_model_outputs"
    ).fetchone()[0]
...
def test_run_pipeline_pre_draft_sidecar_enrichment_merges_raw_columns(
