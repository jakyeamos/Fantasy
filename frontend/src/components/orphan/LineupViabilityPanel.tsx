      ...
      <CardContent>
        {lineup?.slot_scores.length ? (
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            {lineup.slot_scores.map((slot) => {
              const belowReplacement =
                slot.starter_value < slot.replacement_level
              return (
                <div
          ...
          <p className="text-sm leading-6 text-muted-foreground">
            Lineup scores are unavailable. Re-run the lineup engine to see
            slot-by-slot weakness.
          </p>
        )}
      </CardContent>
    </Card>
