...
export function StartupPickValuationList({
  picks,
}: StartupPickValuationListProps) {
  if (!picks.length) {
    return (
      <Card className="border-dashed border-border/45">
        ...
        <CardContent>
          <p className="text-sm leading-6 text-muted-foreground">
            Startup context exists, but there are no pick valuations cached for
            this league yet.
          </p>
        </CardContent>
      </Card>
    ...
    <Card>
      ...
      <CardContent>
        <div className="divide-y divide-border/40">
          {picks.map((pick) => (
            <div
              key={`${pick.pick_slot}-${pick.pick_slot_number}`}
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                {pick.trade_down_recommended ? (
                  <Badge variant="outline">Trade Down</Badge>
                ) : null}
              </div>
              <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                <span className="font-mono">
                  Value {pick.pick_value.toFixed(1)}
                </span>
                {pick.trade_reasoning ? (
                  <span>{pick.trade_reasoning}</span>
                ) : null}
              </div>
            </div>
          ))}
