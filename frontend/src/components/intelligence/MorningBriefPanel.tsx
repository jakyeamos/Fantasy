    ...
    <article className="rounded-lg border border-border/60 bg-background/35 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            {item.league_id ? <Badge variant="outline">{item.league_id}</Badge> : null}
            <Badge variant={item.lane === "watch" ? "outline" : "secondary"}>
              {confidence}% confidence
            </Badge>
          </div>
          <h4 className="font-headline text-lg font-bold leading-tight">{item.headline}</h4>
        </div>
      ...
      {item.recommended_action ? (
        <p className="mt-3 rounded-md border border-primary/20 bg-primary/5 p-3 text-sm">
          <span className="font-semibold">Next move:</span> {item.recommended_action}
        </p>
      ) : null}
      ...
      <div className="mt-3 flex flex-wrap gap-2 border-t border-border/40 pt-3">
        {(["acted", "useful", "not_relevant", "dismissed"] as const).map((verdict) => (
          <Button
            key={verdict}
            size="sm"
...
function ReviewClaim({ event }: { event: FootballEvent }) {
  ...
  const review = useMutation({
    ...
    onSuccess: async () => {
      client.setQueryData<FootballEvent[]>(["v2", "intelligence", "review"], (current) =>
    ...
    <article className="rounded-md border border-border/60 p-3 text-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {event.verification_state.replaceAll("_", " ")} · {Math.round(event.confidence * 100)}%
            confidence
          </p>
        </div>
      ...
      {!event.player_id ? (
        <p className="mt-2 text-xs text-muted-foreground">
          Identity is unresolved; confirmation stays disabled until the player can be resolved.
        </p>
      ) : null}
    </article>
  )
        ...
        <CardHeader className="gap-4 bg-gradient-to-br from-primary/10 via-card to-card">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0 space-y-2">
              ...
              <CardTitle id="morning-brief-title" className="font-headline text-3xl">
                Today
              </CardTitle>
              <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                Fresh football events translated into league-specific moves. Nothing here submits a
                waiver, trade, or lineup change.
              </p>
            </div>
            <Button
          ...
          <div className="flex flex-wrap items-start gap-3 text-xs text-muted-foreground">
            {brief.data ? (
              ...
              <span>
                No verified briefing exists for today. Run it before relying on this page.
              </span>
            )}
          </div>
          {refresh.isError ? (
              ...
              <CardContent className="space-y-3">
                {grouped[lane].length ? (
                  grouped[lane].map((item) => (
                    <BriefCard key={item.item_id} item={item} briefId={brief.data.brief_id} />
                  ))
                ) : (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
            ...
            {analysisNote ? (
              <p className="mt-3 text-sm text-muted-foreground">{analysisNote}</p>
            ) : null}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            {(brief.data?.source_health ?? []).map((source) => (
              <div key={source.source_id} className="border-b border-border/40 pb-2">
                <div className="flex items-center justify-between gap-3">
                  <span className="truncate">{source.source_id}</span>
                  <Badge variant={source.status === "complete" ? "secondary" : "outline"}>
                    {source.status}
                  </Badge>
                </div>
                <p className="mt-1 text-xs text-muted-foreground">
