...
function ResearchPage() {
  ...
  const cards = (query.data?.items ?? [])
    .slice(0, 4)
    .map(decisionCardFromOpportunity)
    ...
    <div className="space-y-8">
      <section className="grid gap-6 border-b border-border/45 pb-8 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-3">
          ...
          <p className="max-w-3xl text-sm leading-6 text-muted-foreground">
            Market signals, rookie evidence, prospect models, and draft context
            are research inputs. The league and roster context decides whether a
            signal becomes a move.
          </p>
        </div>
        <Card>
          <CardHeader>
      ...
      <section className="space-y-4">
        ...
        {!query.isLoading && !query.isError && !cards.length ? (
          <StatePanel
            state={query.data?.status === "degraded" ? "degraded" : "empty"}
          />
        ) : null}
        {cards.length ? (
