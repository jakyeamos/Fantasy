...
import { dashboardSummaryOptions, decisionCardsOptions, todayBriefOptions } from "@/api/queries"
      ...
      <section className="grid gap-6 border-b border-border/45 pb-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-4">
          ...
          <p className="max-w-2xl text-base leading-7 text-muted-foreground">
            Verified local league evidence becomes a small set of reversible next moves. Every card
            keeps its timing, confidence, freshness, downside, and evidence attached.
          </p>
          <div className="flex flex-wrap gap-3">
            <Link to="/leagues" className={buttonClasses({ size: "lg" })}>
              Open leagues <ArrowRight className="size-4" />
            </Link>
            <Link to="/operations" className={buttonClasses({ variant: "outline", size: "lg" })}>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="font-mono font-semibold capitalize">
                {briefQuery.isLoading ? "loading" : (briefQuery.data?.status ?? "not run")}
              </span>
            </div>
        ...
        <section aria-labelledby="decision-list-title" className="space-y-4">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h3 id="decision-list-title" className="mt-2 font-headline text-2xl font-extrabold">
      ...
      <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <Card className="h-fit">
          ...
          <CardContent className="space-y-4 text-sm leading-6 text-muted-foreground">
            <p>
              Source refreshes, URL analysis, and review decisions write only to the local evidence
              store. They never transmit a fantasy action.
            </p>
            <Link
              to="/operations"
