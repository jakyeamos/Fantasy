...
export type OpportunityActionFilter = "all" | OpportunityFeedItem["suggested_action"]
...
export function filterOpportunityItems(
  ...
  return items.filter((item) => {
    ...
    if (filters.scopeFilter !== "all" && item.availability !== filters.scopeFilter) {
      return false
    }
    if (filters.actionFilter !== "all" && item.suggested_action !== filters.actionFilter) {
      return false
    }
