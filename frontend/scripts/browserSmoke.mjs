      ...
      if (response.ok()) {
        ...
        if (first?.league_id) {
          return {
            ...
            rosterId: first.user_roster_id
              ? Number(first.user_roster_id)
              : null,
          }
        }
      }
...
function buildRoutes(league) {
  ...
  routes.push(
    ...
    route(
      "/opportunities?smokeState=stale",
      ["Opportunity Feed", "Stale weekly data"],
      "stale",
    ),
    route(
...
async function installFixture(page, fixture) {
  ...
  await page.route(opportunityApiPattern, async (route) => {
    ...
    const payload = {
      ...
      degraded_reason:
        fixture === "degraded"
          ? "Fixture source is partially unavailable."
          : null,
    }
    await route.fulfill({
      status: 200,
...
async function assertRoute(context, target, consoleProblems) {
  ...
  trackConsoleProblems(
    page,
    consoleProblems,
    target.fixture === "error" ? ["net::ERR_FAILED"] : [],
  )
  ...
  try {
    ...
    const missing = target.expected.filter(
      (text) => !normalizedBody.includes(text.toLowerCase()),
    )
    const overlayCount = await page
      .locator(
        "text=/Plugin: vite|Failed to load|Unhandled|TypeError|ReferenceError/i",
      )
      .count()
